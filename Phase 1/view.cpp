	...
	if (!strcmp(first, "view") & second == NULL) // if inputs total format is ok
	{
		*flagg = 1;//inputs total format is ok
		...
		else//it means that our loged in user was buyer
		{
			...
			for (int i = 0;i < (*buyersaddress)[loged_in_user_number].numofgoods;i++) {//search in goods
				...
				for (int i = 0;i < numofseller;i++)//search in sellers
					for (int j = 0;j < (*sellersaddress)[i].numofgoods;j++)//search in goods
						if (!strcmp((*buyersaddress)[loged_in_user_number].buyers_goods[i].goods_name, (*sellersaddress)[i].sellers_goods[j].goods_name))//goods seller found
							printf("this good's seller: %s\n\n", (*sellersaddress)[i].username);
			}
			free(first);//free command line
		}
	}
